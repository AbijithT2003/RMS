...
export const applicationsApi = {
  ...
  getApplicationsByApplicant: async (applicantId, page = 0, size = 10) => {
    const res = await apiClient.get(`/applications/applicant/${applicantId}`, {
      params: { page, size },
    });
    return res.data?.data || res.data; // Returns PageResponse<JobApplicationResponse>
  },
  ...
  assignRecruiter: async (id, recruiterId) => {
    const res = await apiClient.patch(`/applications/${id}/assign`, null, {
      params: { recruiterId },
    });
    return res.data?.data || res.data; // Returns JobApplicationResponse
  },
  ...
  getApplicantById: async (applicantId) => {
    const res = await apiClient.get(`/applicants/${applicantId}`);
    return res.data?.data || res.data;
  },
